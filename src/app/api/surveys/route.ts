...
export async function GET(request: NextRequest) {
  try {
    ...
    try {
      ...
      // 폴백 데이터 반환
      return NextResponse.json(fallbackSurveys);
    }
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      console.error('Database error, creating survey with fallback:', dbError);

      // 데이터베이스 오류 시 로컬 저장 모드
      // 클라이언트에서 localStorage에 저장하도록 지시
      const tempSurveyId = `temp_${Date.now()}_${Math.random().toString(36).slice(2)}`;
      const tempSurvey = {
        id: tempSurveyId,
        title,
        description,
        ...
        offline: true, // 오프라인 설문임을 표시
      };

      return NextResponse.json(tempSurvey, { status: 201 });
    }
  } catch (error) {
